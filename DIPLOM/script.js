class ImageAnnotationTool {
    constructor() {
        ...
        this.elements = {
            ...
            multipleFilesUpload: document.getElementById('multipleFilesUpload'),
            folderUpload: document.getElementById('folderUpload'),
            singleImageRadio: document.getElementById('singleImageRadio'),
            multipleFilesRadio: document.getElementById('multipleFilesRadio'),
            ...
            multipleFilesLabel: document.getElementById('multipleFilesLabel'),
    ...
    setupNavigation() {
        if (this.elements.prevImage) {
            this.elements.prevImage.addEventListener('click', () => this.previousImage());
        }
    ...
    updateNavigationUI() {
        if (this.images.length <= 1) {
            if (this.elements.navigationPanel) this.elements.navigationPanel.style.display = 'none';
            return;
        }
        
        if (this.elements.navigationPanel) this.elements.navigationPanel.style.display = 'block';
        if (this.elements.imageCounter) {
            this.elements.imageCounter.textContent = `${this.currentImageIndex + 1} / ${this.images.length}`;
        }
    ...
    updateThumbnails() {
        if (!this.elements.imageThumbnails) return;
        
        this.elements.imageThumbnails.innerHTML = '';
        this.images.forEach((img, idx) => {
            const thumbContainer = document.createElement('div');
            thumbContainer.style.position = 'relative';
            thumbContainer.style.cursor = 'pointer';
            thumbContainer.style.margin = '0 5px';
            
            const thumb = document.createElement('img');
            thumb.src = img.url;
            thumb.className = 'thumbnail';
            if (idx === this.currentImageIndex) thumb.classList.add('active');
            ...
            this.elements.imageThumbnails.appendChild(thumbContainer);
        });
    }
    
    async initDB() {
        return new Promise((resolve, reject) => {
            const request = indexedDB.open('AnnotationToolDB', 5);
            ...
            getRequest.onsuccess = () => {
                ...
                if (imageData) {
                    ...
                    putRequest.onsuccess = () => {
                        console.log(`💾 Разметка обновлена для ${imageData.name}`);
                        resolve();
                    };
    ...
    async saveSession() {
        ...
        try {
            ...
            if (this.elements.multipleFilesRadio && this.elements.multipleFilesRadio.checked) activeMode = 'files';
            if (this.elements.multipleImagesRadio && this.elements.multipleImagesRadio.checked) activeMode = 'folder';
    ...
    async restoreSession() {
        ...
        try {
            ...
            if (session.mode === 'files') {
                if (this.elements.multipleFilesRadio) this.elements.multipleFilesRadio.checked = true;
                ...
                this.updateUIMode('files');
            } else if (session.mode === 'folder') {
                if (this.elements.multipleImagesRadio) this.elements.multipleImagesRadio.checked = true;
                if (this.elements.singleImageRadio) this.elements.singleImageRadio.checked = false;
                if (this.elements.multipleFilesRadio) this.elements.multipleFilesRadio.checked = false;
                this.updateUIMode('folder');
            } else {
                if (this.elements.singleImageRadio) this.elements.singleImageRadio.checked = true;
                this.updateUIMode('single');
            }
    ...
    updateUIMode(mode) {
        if (mode === 'single') {
            ...
            if (this.elements.multipleFilesLabel) this.elements.multipleFilesLabel.style.display = 'none';
            ...
            if (this.elements.multipleFilesLabel) this.elements.multipleFilesLabel.style.display = 'inline-block';
            if (this.elements.folderUploadLabel) this.elements.folderUploadLabel.style.display = 'none';
            if (this.images.length > 1 && this.elements.navigationPanel) {
                this.elements.navigationPanel.style.display = 'block';
            }
        } else if (mode === 'folder') {
            if (this.elements.singleUploadLabel) this.elements.singleUploadLabel.style.display = 'none';
            if (this.elements.multipleFilesLabel) this.elements.multipleFilesLabel.style.display = 'none';
    ...
    setupEventListeners() {
        ...
        if (this.elements.singleUpload) {
            this.elements.singleUpload.addEventListener('change', (e) => {
                if (this.isLoading) return;
                this.loadSingleImage(e);
                e.target.value = '';
            });
        }
        
        // Загрузка нескольких файлов (выбранных вручную)
        if (this.elements.multipleFilesUpload) {
            this.elements.multipleFilesUpload.addEventListener('change', (e) => {
                if (this.isLoading) return;
                this.loadMultipleFiles(e);
                e.target.value = '';
            });
        }
        ...
        if (this.elements.folderUpload) {
            this.elements.folderUpload.addEventListener('change', (e) => {
                if (this.isLoading) return;
                this.loadFolderImages(e);
                e.target.value = '';
            });
        }
        ...
        if (this.elements.singleUploadLabel) {
            const newLabel = this.elements.singleUploadLabel.cloneNode(true);
            this.elements.singleUploadLabel.parentNode.replaceChild(newLabel, this.elements.singleUploadLabel);
            this.elements.singleUploadLabel = newLabel;
            this.elements.singleUploadLabel.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
        ...
        if (this.elements.multipleFilesLabel) {
            const newLabel = this.elements.multipleFilesLabel.cloneNode(true);
            this.elements.multipleFilesLabel.parentNode.replaceChild(newLabel, this.elements.multipleFilesLabel);
            this.elements.multipleFilesLabel = newLabel;
            this.elements.multipleFilesLabel.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                if (this.elements.multipleFilesUpload) this.elements.multipleFilesUpload.click();
            });
        }
        
        if (this.elements.folderUploadLabel) {
            const newLabel = this.elements.folderUploadLabel.cloneNode(true);
            this.elements.folderUploadLabel.parentNode.replaceChild(newLabel, this.elements.folderUploadLabel);
            this.elements.folderUploadLabel = newLabel;
            this.elements.folderUploadLabel.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
        ...
        if (this.elements.multipleFilesRadio) {
            this.elements.multipleFilesRadio.addEventListener('change', () => {
                this.updateUIMode('files');
                this.saveSession();
            });
        }
    ...
    async loadSingleImage(event) {
        ...
        console.log('📂 Загрузка одного изображения:', file.name);
    ...
    async loadMultipleFiles(event) {
        ...
        console.log(`📑 Загрузка ${files.length} выбранных файлов`);
    ...
    updateProgressInfo() {
        if (!this.elements.progressInfo) return;
        
        const annotatedCount = this.images.filter(img => img.annotation !== null).length;
        if (this.elements.processedCount) this.elements.processedCount.textContent = annotatedCount;
        if (this.elements.progressBar) {
            this.elements.progressBar.value = (annotatedCount / this.images.length) * 100;
        }
    }
    ...
    async onMouseUp(e) {
        ...
        if (Math.abs(ann.width) > 5 && Math.abs(ann.height) > 5) {
            ...
            console.log('✅ Разметка сохранена в БД');
            this.updateProgressInfo();
            this.updateNavigationUI();
        }
    ...
    async exportAllAnnotations() {
        const annotatedImages = this.images.filter(img => img.annotation !== null);
        if (annotatedImages.length === 0) {
            alert('⚠️ Нет размеченных изображений для экспорта');
            return;
        }
        
        console.log(`📦 Экспорт ${annotatedImages.length} разметок`);
        
        const data = {
            exportDate: new Date().toISOString(),
            totalImages: this.images.length,
            annotatedImages: annotatedImages.length,
            annotations: annotatedImages.map(img => ({
                imageName: img.name,
                annotation: img.annotation
            }))
        };
        ...
        if (this.elements.saveCropped && this.elements.saveCropped.checked) {
            alert(`📦 Экспортировано ${annotatedImages.length} разметок. Сохраняю фрагменты...`);
            for (let i = 0; i < annotatedImages.length; i++) {
                await this.saveCroppedImage(annotatedImages[i].annotation, annotatedImages[i]);
                await new Promise(resolve => setTimeout(resolve, 100));
            }
            alert(`✅ Сохранено ${annotatedImages.length} фрагментов!`);
        } else {
            alert(`📦 Экспортировано ${annotatedImages.length} разметок`);
        }
    }
